...
@ContextConfiguration(locations = "classpath*:excel/spring.xml")
public class ExcelWriterTest {
	...
	public void test2() throws Exception {
		try {
			...
			WriteStatus ws = report2ExcelWriter.write(new FileOutputStream(
					new File("D:/test.xlsx")), beans);
			for (String str : ExcelKit.getInstance().getWriteStatusMessages(ws,
					Locale.CHINESE)){
				System.out.println(str);
	...
	public void test3() throws Exception {
		...
		WriteStatus ws = report3ExcelWriter.write(new FileOutputStream(
				new File("D:/test.xlsx")), beans);
		for (String str : ExcelKit.getInstance().getWriteStatusMessages(ws,
				Locale.CHINESE))
			System.out.println(str);
	...
	public void test4() throws Exception {
		...
		beans.put("list", list);
		WriteStatus ws = report4ExcelWriter.write(new FileOutputStream(
				new File("D:/test.xlsx")), beans);
		for (String str : ExcelKit.getInstance().getWriteStatusMessages(ws,
				Locale.CHINESE))
			System.out.println(str);
	...
	public void test5() throws Exception {
		Map<String, Object> beans = new HashMap<String, Object>();
		for (int i = 1; i <= 14; i++) {
			beans.put("v" + i, new BigDecimal(Math.random() * 60).setScale(2,
					BigDecimal.ROUND_HALF_UP));
		}
		WriteStatus ws = report5ExcelWriter.write(new FileOutputStream(
				new File("D:/test.xlsx")), beans);
		for (String str : ExcelKit.getInstance().getWriteStatusMessages(ws,
				Locale.CHINESE))
			System.out.println(str);
	...
	public void testGenerateCalendar() throws Exception {
		...
		while (c.get(Calendar.YEAR) < 2014) {
			...
			if (c.get(Calendar.DAY_OF_MONTH) == 1
				...
				List<Map<String, Object>> weeks = (List<Map<String, Object>>) bean
						.get("weeks");
				weeks.add(oneweek);
			}
		...
		WriteStatus ws = calendarWriter.writePerSheet(new FileOutputStream(
				new File("D:/calendar.xlsx")), listBeans);
		for (String str : ExcelKit.getInstance().getWriteStatusMessages(ws,
				Locale.CHINESE))
			System.out.println(str);
		System.out.println("Done!");
