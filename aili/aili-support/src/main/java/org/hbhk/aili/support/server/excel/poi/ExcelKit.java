...
public class ExcelKit {
	...
	public String getReadStatusMessages(ExcelManipulateException e,
			Locale locale) {
		return getMessage(EXCEL_EXCEPTION_PREFIX + "." + e.getErrorCode(),
				locale, e.getArgs());
	}

	public List<String> getReadStatusMessages(ReadStatus readStatus,
			Locale locale) {
		List<String> result = new ArrayList<String>();
		result.add(getMessage(
				READ_STATUS_PREFIX + "." + readStatus.getStatus(), locale, null));
			...
			if (readStatus.getStatus() == ReadStatus.STATUS_DATA_COLLECTION_ERROR) {
				for (Exception e : readStatus.getExceptions()) {
					if (e instanceof ExcelManipulateException) {
						...
						result.add(getMessage(EXCEL_EXCEPTION_PREFIX + "."
								+ eme.getErrorCode(), locale, eme.getArgs()));
					}
				}
			} else
	...
	public List<String> getWriteStatusMessages(WriteStatus writeStatus,
			Locale locale) {
		List<String> result = new ArrayList<String>();
		result.add(getMessage(
				WRITE_STATUS_PREFIX + "." + writeStatus.getStatus(), locale,
				null));
