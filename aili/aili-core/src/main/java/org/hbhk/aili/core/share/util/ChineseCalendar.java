...
public class ChineseCalendar {
	...
	private static String[] chineseMonthNames = { "正", "二", "三", "四", "五", "六",
			"七", "八", "九", "十", "冬", "腊" };
	private static String[] chineseDayNames = { "初一", "初二", "初三", "初四", "初五",
	...
	public String getChineseMonthName() {
		if (chineseMonth > 0) {
			return chineseMonthNames[chineseMonth - 1] + "月";
		} else
			return "闰" + chineseMonthNames[-chineseMonth - 1] + "月";
	}

	public String getChineseDayName() {
		return chineseDayNames[chineseDate - 1];
	}

	public String getChineseDayNameWithMonth() {
		return getChineseMonthName() + getChineseDayName();
	}

	public String getShortChineseDayNameWithMonth() {
		if (chineseDate == 1)
			return getChineseMonthName();
		else
			return getChineseDayName();
	}
	...
	public Calendar getGregorianCalendar() {
		...
		c.set(Calendar.YEAR, gregorianYear);
		c.set(Calendar.MONTH, gregorianMonth - 1);
		c.set(Calendar.DATE, gregorianDate);
		return c;
	}
